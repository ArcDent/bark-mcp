  ...
  "mcp": {
    "bark": {
      ...
      "environment": {
        ...
        "BARK_TITLE_QUESTION": "OpenCode 提出问题",
...
- 权限申请通知：正文使用 `<中文翻译>|<权限申请内容>`，例如 `执行命令|bash: npm test`
- 问题通知：正文使用问题的 `header`、`title` 或 `question`
- 会话完成通知：正文最多取最后一条 assistant 文本前 100 个字符；超过 100 个字符时在末尾追加 `......`
...
- `BARK_TITLE_QUESTION`：仅用于自动通知插件的“提出问题 / 等待回复”标题；未配置时回退到 `BARK_TITLE`
